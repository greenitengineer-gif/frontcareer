...
exports.updateCV = exports.getMyCV = exports.setPrimaryCV = exports.getAllUserCVs = exports.getPublicCVs = exports.getCVByUserId = exports.getCV = void 0;
const supabase_1 = require("../utils/supabase");
const crypto_1 = __importDefault(require("crypto"));
const mapLanguageLevelFromServer = (level) => {
    if (!level)
        return undefined;
    if (level === 'elementary')
        return 'basic';
...
const formatCVForFrontend = (cv) => {
    ...
    return {
        ...
        skills: cvSkills.map((s) => ({
            id: s.id,
            cvId: s.cv_id,
            name: s.skill_name,
            level: s.level,
        })),
        languages: cvLanguages.map((l) => ({
            id: l.id,
            cvId: l.cv_id,
...
exports.getCVByUserId = getCVByUserId;
const getPublicCVs = async (req, res) => {
    try {
        const { data: cvs, error: cvError } = await supabase_1.supabase
            .from('cvs')
            .select(`
        *,
        user:users(*),
...
exports.getPublicCVs = getPublicCVs;
const getAllUserCVs = async (req, res) => {
    try {
...
exports.getAllUserCVs = getAllUserCVs;
...
const updateCV = async (req, res) => {
    try {
        ...
        const { data: existingCV } = await supabase_1.supabase
            .from('cvs')
            .select('id')
            .eq('user_id', userId)
            .maybeSingle();
        const cvUpsertData = {
            id: existingCV?.id || crypto_1.default.randomUUID(),
            user_id: userId,
            updated_at: new Date().toISOString(),
        };
        if (title !== undefined)
            cvUpsertData.title = title;
        if (firstName !== undefined)
            cvUpsertData.first_name = firstName;
        if (lastName !== undefined)
            cvUpsertData.last_name = lastName;
        if (template !== undefined)
            cvUpsertData.template = template;
        if (brandColor !== undefined)
            cvUpsertData.brand_color = brandColor;
        if (summary !== undefined)
            cvUpsertData.summary = summary;
        if (birthDate !== undefined)
            cvUpsertData.birth_date = birthDate;
        if (gender !== undefined)
            cvUpsertData.gender = gender;
        if (address !== undefined)
            cvUpsertData.address = address;
        if (expectedSalary !== undefined)
            cvUpsertData.expected_salary = expectedSalary;
        if (isPublic !== undefined)
            cvUpsertData.is_public = isPublic;
        if (image !== undefined)
            cvUpsertData.image = image;
        ...
        if (firstName || lastName)
            userUpdateData.name = `${lastName || ''} ${firstName || ''}`.trim();
        if (image)
            userUpdateData.avatar = image;
        // body.phone is the phone from the CV builder
        if (body.phone)
            userUpdateData.phone = body.phone;
        if (body.jobTitle)
            userUpdateData.job_title = body.jobTitle;
        if (Object.keys(userUpdateData).length > 0) {
            const { error: userUpdateError } = await supabase_1.supabase.from('users').update(userUpdateData).eq('id', userId);
            if (userUpdateError) {
        ...
        try {
            const { data: userData, error: userFetchError } = await supabase_1.supabase.from('users').select('*').eq('id', userId).single();
        ...
        if (Array.isArray(education)) {
            const { error: deleteError } = await supabase_1.supabase.from('education').delete().eq('cv_id', cvId);
            if (deleteError) {
            ...
            if (education.length > 0) {
                ...
                const { error: eduError } = await supabase_1.supabase.from('education').insert(eduData);
                if (eduError) {
        ...
        if (Array.isArray(experience)) {
            const { error: deleteError } = await supabase_1.supabase.from('experience').delete().eq('cv_id', cvId);
            if (deleteError) {
            ...
            if (experience.length > 0) {
                ...
                const { error: expError } = await supabase_1.supabase.from('experience').insert(expData);
                if (expError) {
        ...
        if (Array.isArray(skills)) {
            const { error: deleteError } = await supabase_1.supabase.from('cv_skills').delete().eq('cv_id', cvId);
            if (deleteError) {
            ...
            if (skills.length > 0) {
                ...
                const { error: skillError } = await supabase_1.supabase.from('cv_skills').insert(skillData);
                if (skillError) {
        ...
        if (Array.isArray(languages)) {
            const { error: deleteError } = await supabase_1.supabase.from('cv_languages').delete().eq('cv_id', cvId);
            if (deleteError) {
            ...
            if (languages.length > 0) {
                ...
                const { error: langError } = await supabase_1.supabase.from('cv_languages').insert(langData);
                if (langError) {
        ...
        if (Array.isArray(certificates)) {
            const { error: deleteError } = await supabase_1.supabase.from('certificates').delete().eq('cv_id', cvId);
            if (deleteError) {
            ...
            if (certificates.length > 0) {
                ...
                const { error: certError } = await supabase_1.supabase.from('certificates').insert(certData);
                if (certError) {
        ...
        if (Array.isArray(projects)) {
            await supabase_1.supabase.from('projects').delete().eq('cv_id', cvId);
            if (projects.length > 0) {
                ...
                await supabase_1.supabase.from('projects').insert(projectData);
            }
        }
        // 8. Handle Awards
        if (Array.isArray(awards)) {
            await supabase_1.supabase.from('awards').delete().eq('cv_id', cvId);
            if (awards.length > 0) {
                ...
                await supabase_1.supabase.from('awards').insert(awardData);
            }
        }
        // 9. Handle Volunteers
        if (Array.isArray(volunteers)) {
            await supabase_1.supabase.from('volunteers').delete().eq('cv_id', cvId);
            if (volunteers.length > 0) {
                ...
                await supabase_1.supabase.from('volunteers').insert(volunteerData);
            }
        }
        // 10. Handle Hobbies
        if (Array.isArray(hobbies)) {
            await supabase_1.supabase.from('hobbies').delete().eq('cv_id', cvId);
            if (hobbies.length > 0) {
                const hobbyData = hobbies.map((h) => ({
                    id: crypto_1.default.randomUUID(),
                    cv_id: cvId,
                    name: h.name,
                }));
                await supabase_1.supabase.from('hobbies').insert(hobbyData);
            }
        }
        // 11. Handle Memberships
        if (Array.isArray(memberships)) {
            await supabase_1.supabase.from('memberships').delete().eq('cv_id', cvId);
            if (memberships.length > 0) {
                ...
                await supabase_1.supabase.from('memberships').insert(membershipData);
            }
        }
